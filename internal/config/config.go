...
type Config struct {
	PSTRootPath        string   `yaml:"pst_root_path" json:"pst_root_path"`
	PollIntervalMin    int      `yaml:"poll_interval_minutes" json:"poll_interval_minutes"`
	SafeDelayMin       int      `yaml:"safe_delay_minutes" json:"safe_delay_minutes"`
	RetainDays         int      `yaml:"retain_days" json:"retain_days"`
	MaxBatchSize       int      `yaml:"max_batch_size" json:"max_batch_size"`
	ArchiveMode        string   `yaml:"archive_mode" json:"archive_mode"` // "all" | "list"
	ExcludeFolders     []string `yaml:"exclude_folders" json:"exclude_folders"`
	IncludeFolders     []string `yaml:"include_folders" json:"include_folders"`
	LogRetentionDays   int      `yaml:"log_retention_days" json:"log_retention_days"`
	MoveIntervalMs     int      `yaml:"move_interval_ms" json:"move_interval_ms"`
	DryRun             bool     `yaml:"dry_run" json:"dry_run"`
	CopyOnly           bool     `yaml:"copy_only" json:"copy_only"`
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			if err := ValidateConfig(config); err != nil {
				return nil, isFirstRun, fmt.Errorf("invalid default configuration: %w", err)
			}
			return config, isFirstRun, nil
		}
